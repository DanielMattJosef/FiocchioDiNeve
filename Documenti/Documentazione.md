...
### Scopo
  ...
  Come spiegato nell'abstract il progetto permettera di "creare" un fiocco di neve tramite un'applicazione JAVA:
  partendo da un triangolo bianco posizioneremo dei punti (dei poligoni) che verranno sottratti (tagliati via) dal triangolo. Durante questo "tagliuzzamento" si potra assistere alla generazione in live di questo fiocco.
...
### Analisi del dominio
  ...
  Oggi per creare un fiocco di neve, prendiamo un foglio e lo pieghiamo in 8 o piú parti fino a ricavare un triangolo,poi con una forbice lo tagliuzzeremo con strane forme. Dopo il taglio del foglio lo si apre ed ecco comparire il fiocco di neve.
  Questa applicazione é stata sviluppata in JAVA 12.0.1 utilizzando Netbeans 11.1 e lo so riesce a far funzionare su tutti i sistemi operativi.
  Su internet si trovano alcune versioni online di questo progetto scritto in JavaScript.
  Gli utilizzatori di questa applicazione potranno durante momenti morti, ma anche durante una pausa dal lavoro in cui non si dispone di Internet di dilettarsi nella creazione di fiocchi di neve, per poi salvarli e mostrarli a familiari e amici, utilizzando qualsiasi sistema operativo.
